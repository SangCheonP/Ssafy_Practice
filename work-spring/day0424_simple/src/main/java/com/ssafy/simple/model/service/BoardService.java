...
public class BoardService {
	...
	public int write(BoardDTO board, MultipartFile[] files) throws SQLException, IllegalStateException, IOException {
		...
		bdao.insert(board); // 파일 첨부해서 디비에 기록하려면 지금 작성한 게시글 번호가 필요함.
	...
	// 서비스 여기에서 조회수 관리 같은거 해야함. 너네는 하거라.
	public BoardDTO read(int bno) {
		BoardDTO board = bdao.selectOne(bno); // 게시글 꺼내왔고
		board.setFiles(fdao.selectList(bno)); // 그 게시글에 첨부된 파일목록 꺼내서 board 넣고
		return board;
	}
